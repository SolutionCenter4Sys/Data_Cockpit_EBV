    ...
    <Box>
      ...
      {anomalyCount > 0 && (
        <MuiAlert severity="warning" icon={<WarningAmberIcon />} sx={{ mb:2 }}>
          <strong>{anomalyCount} anomalia(s) detectada(s)</strong> pelo Sentinela e qualificadas para investigacao pelo AG-02 Detetive.
        </MuiAlert>
      )}

      {!baselineLearned && (
        <MuiAlert severity="info" sx={{ mb:2 }}>
          Aprendizado de baseline em andamento. Aguardar conclusao para deteccao precisa de anomalias.
        </MuiAlert>
      )}
      ...
      <Grid container spacing={2} mb={3}>
        {[
          ...
          { icon:<FilterAltIcon />, title:"Filtro de Ruido", desc:`${noiseReduction}% de reducao ativa — ${monitoredSources * 47} sinais filtrados por ciclo`, ok:true },
          ...
          { icon:<WarningAmberIcon />, title:"Saida para AG-02", desc:`${anomalyCount} sinais qualificados como anomalias — aguardando investigacao`, ok:anomalyCount === 0 },
        ].map((item, i) => (
          <Grid item xs={12} sm={6} md={3} key={i}>
            <Card sx={{ borderRadius:2, height:"100%", border:`1px solid ${item.ok ? theme.palette.success.main + "30" : theme.palette.warning.main + "30"}` }}>
              <CardContent sx={{ p:2 }}>
                <Box sx={{ display:"flex", alignItems:"center", gap:1, mb:1 }}>
                  <Box sx={{ color: item.ok ? theme.palette.success.main : theme.palette.warning.main }}>
                    {item.icon}
                  </Box>
                  <Typography variant="subtitle2" fontWeight={700}>{item.title}</Typography>
                  {item.ok ? <CheckCircleIcon sx={{ ml:"auto", color:theme.palette.success.main, fontSize:16 }} />
            ...
            <TableBody>
              {signals.map((sig) => (
                <TableRow key={sig.id} hover sx={{
                  bgcolor: sig.isAnomaly ? (isLight ? "rgba(227,24,55,0.03)" : "rgba(227,24,55,0.07)") : "transparent"
                }}>
                  <TableCell>
                    <Typography variant="body2" fontWeight={600}>{sig.source}</Typography>
                  </TableCell>
                  <TableCell>
                    <Chip label={sig.layer} size="small"
                      sx={{ bgcolor: LAYER_COLOR[sig.layer] + "20", color: LAYER_COLOR[sig.layer], fontWeight:600, border:"none" }} />
                  </TableCell>
                  ...
                  <TableCell>
                    <Chip label={sig.quality} size="small" color={QUALITY_COLOR[sig.quality]} variant="outlined" />
                  </TableCell>
                  <TableCell>
                    {sig.isAnomaly
