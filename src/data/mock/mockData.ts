...
export const MOCK_ALERTS: Alert[] = [
  ...
  { id: 'ALT-001', title: 'Falha Crítica — Job ETL-047 (Ingestion)', description: 'Job de ingestão ETL-047 falhou após 3 tentativas. Arquivo FFT não foi copiado corretamente.', severity: 'CRITICAL', layer: 'INGESTION', stage: 'INGESTAO', area: 'Engineering', triggeredAt: '2026-03-29T07:58:10Z', status: 'OPEN', triggerType: 'BATCH_FAILURE', affectedProcess: 'ETL-047 FFT Copy Job', suggestedAction: 'Reexecutar manualmente o job ETL-047 com validação de checksum.' },
  { id: 'ALT-002', title: 'Inconsistência — Variável Endereço (Enriquecimento)', description: 'Taxa de inconsistência na variável de endereço atingiu 8.2% na camada de Ingestion.', severity: 'HIGH', layer: 'INGESTION', stage: 'INGESTAO', area: 'Retail', triggeredAt: '2026-03-29T08:45:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'Enrichment Service — Endereço', suggestedAction: 'Investigar fonte de dados de enriquecimento de endereço. Validar API externa.' },

  // ── Governança (1 alerta OPEN — bate com StageHealth GOVERNANCA activeAlerts:1) ──
  { id: 'ALT-023', title: 'Regra de governança violada — classificação PII', description: 'Coluna email detectada sem tag PII obrigatória em 3 tabelas da camada Governança.', severity: 'MEDIUM', layer: 'TRUSTED', stage: 'GOVERNANCA', area: 'Compliance', triggeredAt: '2026-03-29T08:20:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'Governance Tag Validator', suggestedAction: 'Aplicar tags PII faltantes e revisar política de classificação.' },

  // ── DW / Trusted (16 alertas OPEN — bate com StageHealth DW activeAlerts:16) ──
  { id: 'ALT-003', title: 'Latência Elevada — Trusted Layer (340ms)', description: 'Latência da camada Trusted atingiu 340ms, ultrapassando o limiar de 300ms definido.', severity: 'HIGH', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T09:12:00Z', status: 'OPEN', triggerType: 'LATENCY', affectedProcess: 'Trusted Data Validation Service', suggestedAction: 'Escalar capacidade do serviço de validação.' },
  { id: 'ALT-004', title: 'Job Atrasado — BATCH-092 (Trusted)', description: 'Job BATCH-092 está com 45 minutos de atraso em relação ao SLA definido.', severity: 'MEDIUM', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T05:15:00Z', status: 'OPEN', triggerType: 'BATCH_FAILURE', affectedProcess: 'BATCH-092 Trusted Sync', suggestedAction: 'Verificar dependências do job BATCH-092.' },
  { id: 'ALT-005', title: 'Completeness Check falhou — DW tabela financeiro', description: 'Teste de completude retornou 72% (limiar 95%) na tabela fact_financeiro.', severity: 'CRITICAL', layer: 'TRUSTED', stage: 'DW', area: 'Finance', triggeredAt: '2026-03-29T06:10:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'DW Completeness Gate', suggestedAction: 'Verificar job de carga da tabela fact_financeiro.' },
  { id: 'ALT-006', title: 'Freshness SLA violado — dim_produto', description: 'Dados da dimensão produto estão 4h defasados. SLA é 1h.', severity: 'HIGH', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T07:20:00Z', status: 'OPEN', triggerType: 'LATENCY', affectedProcess: 'DW dim_produto Refresh', suggestedAction: 'Reprocessar carga da dim_produto.' },
  { id: 'ALT-007', title: 'Accuracy falhou — fact_transacoes checksum', description: 'Checksum de fact_transacoes diverge da origem em 3.2%.', severity: 'HIGH', layer: 'TRUSTED', stage: 'DW', area: 'Risk', triggeredAt: '2026-03-29T07:45:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'DW Accuracy Validation', suggestedAction: 'Investigar transformação ETL de fact_transacoes.' },
  { id: 'ALT-008', title: 'Null ratio alto — dim_cliente.cpf', description: 'Coluna CPF com 12% de nulos na dimensão cliente (limiar 0%).', severity: 'HIGH', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T08:00:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'DW dim_cliente Quality', suggestedAction: 'Verificar fonte de dados CRM.' },
  { id: 'ALT-009', title: 'Duplicatas detectadas — fact_pagamentos', description: 'Encontradas 1.247 linhas duplicadas no carregamento mais recente.', severity: 'MEDIUM', layer: 'TRUSTED', stage: 'DW', area: 'Finance', triggeredAt: '2026-03-29T08:15:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'DW Dedup Gate', suggestedAction: 'Rodar deduplicação e investigar origem do reprocessamento.' },
  { id: 'ALT-010', title: 'Schema drift detectado — stg_vendas', description: 'Coluna valor_bruto mudou de DECIMAL(10,2) para VARCHAR.', severity: 'HIGH', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T08:30:00Z', status: 'OPEN', triggerType: 'ANOMALY', affectedProcess: 'DW Schema Validator', suggestedAction: 'Reverter schema e alinhar com time de ingestão.' },
  { id: 'ALT-011', title: 'Volume anomaly — fact_vendas -40% vs D-1', description: 'Volume de registros 40% menor que o dia anterior.', severity: 'MEDIUM', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T09:00:00Z', status: 'OPEN', triggerType: 'ANOMALY', affectedProcess: 'DW Volume Monitor', suggestedAction: 'Confirmar com equipe de ingestão se todas as fontes rodaram.' },
  { id: 'ALT-012', title: 'Referential integrity — FK cliente quebrada', description: '89 registros em fact_pedidos sem correspondência em dim_cliente.', severity: 'MEDIUM', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T09:15:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'DW FK Checker', suggestedAction: 'Verificar carga de dim_cliente e reprocessar pedidos órfãos.' },
  { id: 'ALT-013', title: 'Quality gate barrada — camada DW', description: 'Gate de qualidade bloqueou promoção de 3 tabelas para Analytics por falha acumulada.', severity: 'MEDIUM', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T09:30:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'DW Quality Gate', suggestedAction: 'Corrigir os 11 checks falhados e re-executar gate.' },
  { id: 'ALT-014', title: 'Partition missing — fact_financeiro 2026-03-28', description: 'Partição do dia 28/03 não encontrada na tabela fact_financeiro.', severity: 'HIGH', layer: 'TRUSTED', stage: 'DW', area: 'Finance', triggeredAt: '2026-03-29T09:45:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'DW Partition Monitor', suggestedAction: 'Verificar job de carga da partição e reprocessar.' },
  { id: 'ALT-024', title: 'Timeliness SLA — dim_canal 3h atrasado', description: 'Dimensão canal não foi atualizada nas últimas 3h. SLA é 1h.', severity: 'HIGH', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T10:00:00Z', status: 'OPEN', triggerType: 'LATENCY', affectedProcess: 'DW dim_canal Refresh', suggestedAction: 'Verificar job de carga da dim_canal.' },
  { id: 'ALT-025', title: 'Row count anomaly — stg_clientes -60%', description: 'Volume de staging de clientes 60% abaixo do esperado.', severity: 'MEDIUM', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T10:10:00Z', status: 'OPEN', triggerType: 'ANOMALY', affectedProcess: 'DW Volume Monitor stg_clientes', suggestedAction: 'Verificar fonte CRM e job de extração.' },
  { id: 'ALT-026', title: 'Null spike — fact_vendas.valor_liquido', description: 'Coluna valor_liquido com 8% de nulos no último batch (baseline < 0.1%).', severity: 'HIGH', layer: 'TRUSTED', stage: 'DW', area: 'Finance', triggeredAt: '2026-03-29T10:15:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'DW Null Monitor', suggestedAction: 'Investigar transformação de valor_liquido no ETL.' },
  { id: 'ALT-027', title: 'Constraint violation — dim_agencia.codigo duplicado', description: 'Encontrados 23 códigos de agência duplicados após merge incremental.', severity: 'MEDIUM', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-29T10:20:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'DW Uniqueness Gate', suggestedAction: 'Executar dedup na dim_agencia e revisar regra de merge.' },

  // ── Analytics (3 alertas OPEN — bate com StageHealth ANALYTICS activeAlerts:3) ──
  { id: 'ALT-015', title: 'Score Zerado — Modelo MDL-003 (10.3%)', description: 'Percentual de score zerado atingiu o limiar crítico de 10% na base do modelo de Fraude Transacional.', severity: 'CRITICAL', layer: 'ANALYTICS', stage: 'ANALYTICS_STAGE', area: 'Risk', triggeredAt: '2026-03-29T08:14:22Z', status: 'OPEN', triggerType: 'SCORE_ZERO', affectedProcess: 'MDL-003 Score Engine', suggestedAction: 'Verificar processo ETL-047 e reprocessar arquivo FFT corrompido.', autoAction: 'Processo de alertas enviados para equipe de dados via mensageria.' },
  { id: 'ALT-016', title: 'Modelos sem Métricas — Analytics (900+)', description: 'Detectados 912 modelos analíticos sem métricas de monitoramento configuradas.', severity: 'MEDIUM', layer: 'ANALYTICS', stage: 'ANALYTICS_STAGE', area: 'Risk', triggeredAt: '2026-03-29T06:00:00Z', status: 'OPEN', triggerType: 'THRESHOLD', affectedProcess: 'Model Metrics Registry', suggestedAction: 'Mapear e configurar métricas para os 912 modelos não monitorados.' },
  { id: 'ALT-017', title: 'Feature drift — modelo MDL-001', description: 'Drift detectado na feature renda_mensal: distribuição deslocou 1.8 sigma.', severity: 'HIGH', layer: 'ANALYTICS', stage: 'ANALYTICS_STAGE', area: 'Risk', triggeredAt: '2026-03-29T09:02:00Z', status: 'OPEN', triggerType: 'ANOMALY', affectedProcess: 'MDL-001 Feature Monitor', suggestedAction: 'Acionar Data Science para avaliar retreino do modelo.' },

  // ── Delivery (0 alertas OPEN) ──

  // ── Produtos (1 alerta OPEN — bate com StageHealth PRODUTOS activeAlerts:1) ──
  { id: 'ALT-019', title: 'Produto Score PJ — output atrasado', description: 'Output do produto Score PJ está 2h atrasado em relação ao SLA de entrega.', severity: 'MEDIUM', layer: 'ANALYTICS', stage: 'PRODUTOS', area: 'Risk', triggeredAt: '2026-03-29T10:05:00Z', status: 'OPEN', triggerType: 'LATENCY', affectedProcess: 'Produto Score PJ Delivery', suggestedAction: 'Verificar dependências upstream da camada Analytics.' },

  // ── Resolvidos (histórico) ──
  { id: 'ALT-020', title: 'Latência normalizada — Trusted Layer', description: 'Latência voltou ao normal após scaling.', severity: 'MEDIUM', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-28T14:00:00Z', status: 'RESOLVED', triggerType: 'LATENCY', affectedProcess: 'Trusted Data Validation Service', suggestedAction: 'Monitorar.', resolvedAt: '2026-03-28T16:00:00Z' },
  { id: 'ALT-021', title: 'Job ETL-023 recuperado', description: 'Enriquecimento de endereço normalizado após correção na API.', severity: 'HIGH', layer: 'INGESTION', stage: 'INGESTAO', area: 'Retail', triggeredAt: '2026-03-28T09:00:00Z', status: 'RESOLVED', triggerType: 'BATCH_FAILURE', affectedProcess: 'ETL-023 Enrichment', suggestedAction: 'OK.', resolvedAt: '2026-03-28T11:30:00Z' },
  { id: 'ALT-022', title: 'Quality gate passou — DW batch noturno', description: 'Gate de qualidade do batch noturno passou com 100%.', severity: 'LOW', layer: 'TRUSTED', stage: 'DW', area: 'Engineering', triggeredAt: '2026-03-28T04:00:00Z', status: 'RESOLVED', triggerType: 'THRESHOLD', affectedProcess: 'DW Quality Gate', suggestedAction: 'Nenhuma.', resolvedAt: '2026-03-28T04:05:00Z' },
];
